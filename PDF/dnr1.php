...
{ 
	...
	$query = "select DATEDON,STR,idon,IDP,TAS,TAD,POIDS from don WHERE  IDDNR = '$IDDNR'  order by  idon   ";
	$resultat=mysql_query($query);
    ...
    {
	$this->mysqlconnect();
	$query0 = "select * from DNR WHERE  IDDNR = '$idon'";
	...
	$query1 = "select DATEDON,IDP,TAS,TAD,IDDNR,idon from don WHERE  IDDNR='$IDDNR'  order by  idon asc    LIMIT 15, 30 ";
	$resultat1=mysql_query($query1);
	...
	$this->cell(23,05,"TA",1,0,'C',0);
	$this->SetXY(78,20.8); 
	$query2 = "select DATEDON,IDP,TAS,TAD,IDDNR,idon from don WHERE  IDDNR='$IDDNR' order by  idon asc LIMIT 0, 15 ";
	...
	while($row2=mysql_fetch_object($resultat2))
	  {
	   ...
	   $this->cell(23,5,$row2->TAS,1,0,'C',0);
	...
	$query3 = "select DATEDON,IDP,TAS,TAD,IDDNR,idon from don WHERE  IDDNR='$IDDNR' order by idon asc LIMIT 0, 1 ";
	...
	$query4 = "select * from DNR WHERE  IDDNR = '$IDDNR'";
    ...
    {
	$this->mysqlconnect();
	$query1 = "select * from DNR WHERE  IDDNR = '$idon'";
	$resultat1=mysql_query($query1);
	$row = mysql_fetch_array($resultat1); 
	$IDDNR=$row['IDDNR'];
	//1ere page droite
