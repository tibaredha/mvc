...
{ 
	...
	{
    $this->mysqlconnect();
	$sql = " select * from deceshosp where $colone1>=1 and  $colone1<=90  and (DINS BETWEEN '$datejour1' AND '$datejour2')  ";
	$requete = @mysql_query($sql) or die($sql."<br>".mysql_error());
	$x = array(); 
	...
	{    
		...
		$query="SELECT $valeurs,count($valeurs)as nbr,CODECIM,CIM1 FROM deceshosp where DINS BETWEEN '$datejour1' AND '$datejour2' GROUP BY $valeurs  order by nbr desc "; //    % %will search form 0-9,a-z            
		$resultat=mysql_query($query);
		$totalmbr1=mysql_num_rows($resultat);
		while($row=mysql_fetch_object($resultat))
		{
		$this->SetFont('Times', '', 10);
			if ($valeurs=='CIM1') 
			{
				...
				$this->cell(115,4,html_entity_decode(utf8_decode($this->stringtostring('cim','diag_cod',trim($row->$valeurs),'diag_nom'))) ,1,0,'L',0);	
			} 
			// else 
			// {
