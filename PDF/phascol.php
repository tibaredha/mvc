...
$pdf->mysqlconnect();
$idon=$_GET['uc'];
$query = "select * from eleve WHERE  id = '$idon'     ";
$resultat=mysql_query($query);
while($result=mysql_fetch_object($resultat))//
{
$d1=substr($result->DATENS,6,4);$d2=substr(date('d/m/Y'),6,4);$d3=$d2-$d1;
...
$pdf->SetFont('Arial','U');
$pdf->Cell(100,6,$pdf->nbrtostring('mvc','pha16','id',$_SESSION['ordonnace']['libelleProduit'][$i],'pre').'  '.$pdf->nbrtostring('mvc','pha16','id',$_SESSION['ordonnace']['libelleProduit'][$i],'DOSAGE'),0,0,'L',0);
$pdf->SetFont('Arial','');
