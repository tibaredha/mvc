...
$pdf->SetXY(5,$pdf->GetY()+5);$pdf->Cell(200,5,"PROCÉS VERBAL DE CONFORMITÉ D'UN CABINET DE MEDECINE DENTAIRE ",0,1,'C');$pdf->SetFont('aefurat', '', 12);
...
if($rowy->NAT==1){$pdf->Cell(200,5,"Dans un Cadre de Transfert d'un cabinet de médecine dentaire",0,1,'L');}
if($rowy->NAT==2){$pdf->Cell(200,5,"Dans un Cadre d'une Instatllation d'un cabinet de médecine dentaire ",0,1,'L');"";}
if($rowy->NAT==3){$pdf->Cell(200,5,"Dans un Cadre d'une Ouverture d'un cabinet de médecine dentaire ",0,1,'L');"";}
$pdf->SetXY(20,$pdf->GetY()+5);$pdf->Cell(200,5,"Et compte tenu de l'état des lieux visités,à savoir : ",0,1,'L');
$pdf->SetXY(30,$pdf->GetY()+5); $pdf->Cell(200,5,"- Cabinet de soins et de consultation  ",0,1,'L');$pdf->SetXY(115,$pdf->GetY()-5); $pdf->Cell(20,5,"Superficie  : ".$rowy->CDS0." M2",0,1,'L');
$pdf->SetXY(30,$pdf->GetY()+5); $pdf->Cell(200,5,"- Salle de stérilisation  ",0,1,'L');$pdf->SetXY(115,$pdf->GetY()-5); $pdf->Cell(20,5,"Superficie  : ".$rowy->SDS0." M2",0,1,'L');
...
$pdf->SetXY(20,$pdf->GetY()+5);$pdf->Cell(200,5,"J'atteste que le cabinet de médecine dentaire de Mr/Melle/Mme : ".strtoupper($rowx->NOM).'_'.strtolower ($rowx->PRENOM),0,1,'L');
...
$pdf->SetXY(20,$pdf->GetY()+5);$pdf->Cell(200,5,"Est conforme a l'exercice de la profession de médecine dentaire. ",0,1,'L');
$query_listey = "SELECT * FROM home WHERE id  ='$idh' ";$requetey = mysql_query( $query_listey ) or die( "ERREUR MYSQL numéro: ".mysql_errno()."<br>Type de cette erreur: ".mysql_error()."<br>\n" );
while($rowy=mysql_fetch_object($requetey))
{
