...
{
	$pdf->entetepvc("D'UN CABINET DE SOINS GENERAUX","Instruction N°00177/MSP/CAB/ du 11 avril 1987");
	...
	while($rowy=mysql_fetch_object($requetey))
	{
		...
		if($rowy->NAT==1){$pdf->Cell(200,5,"Dans un Cadre de Transfert d'un cabinet de médecine générale ",0,1,'L');}
		if($rowy->NAT==2){$pdf->Cell(200,5,"Dans un Cadre d'une Instatllation d'un cabinet de médecine générale ",0,1,'L');"";}
		if($rowy->NAT==3){$pdf->Cell(200,5,"Dans un Cadre d'une Ouverture d'un cabinet de médecine générale ",0,1,'L');"";}
